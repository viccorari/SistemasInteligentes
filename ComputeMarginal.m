...
E = ObserveEvidence(ComputeJointDistribution(F), E);

[M.var Ei Vi] = intersect(E.var, V);
[dummy, mapM] = ismember(M.var, E.var);
M.card = E.card(Ei);
M.val = zeros(1,prod(M.card));

for i = 1:length(E.val),
    O = IndexToAssignment(i, E.card);
    A = O(mapM);
    p = AssignmentToIndex(A, M.card);
    M.val(p) = M.val(p) + E.val(i);
end;    
